...
void SM64_EXPORT sm64_global_init( uint8_t *rom, uint8_t *outTexture, SM64DebugPrintFunctionPtr debugPrintFunction )
{
...
void SM64_EXPORT sm64_load_surfaces( uint16_t terrainType, const struct SM64Surface *surfaceArray, uint32_t numSurfaces )
{
    surfaces_load_static_libsm64( surfaceArray, numSurfaces );
    gCurrentArea->terrainType = terrainType;
}

void SM64_EXPORT sm64_mario_reset( int16_t marioX, int16_t marioY, int16_t marioZ )
{
...
void SM64_EXPORT sm64_mario_tick( const struct SM64MarioInputs *inputs, struct SM64MarioState *outState, struct SM64MarioGeometryBuffers *outBuffers )
{
...
void SM64_EXPORT sm64_global_terminate( void )
{
...
uint32_t SM64_EXPORT sm64_load_surface_object( const struct SM64SurfaceObject *surfaceObject )
{
    return surfaces_load_object( surfaceObject );
}

void SM64_EXPORT sm64_move_object( uint32_t id, const struct SM64ObjectTransform *transform )
{
    surface_object_update_transform( id, transform );
}

void SM64_EXPORT sm64_unload_object( uint32_t id )
{
}